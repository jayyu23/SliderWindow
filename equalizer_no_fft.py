"""
Equalizer Class: UI Only. No FFT/Audio Parsing
"""
...
class TkWindow(tk.Frame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.x_range = [2**i for i in range(6, 15)]  # Start at 2^6, end at 2^(15 - 1)
        self.y_range = [-17, 3]  # Lowest at -17, highest at +3
    ...
    def export_button_action(self):
        ...
        :return: None
        """
        filename = asksaveasfilename()  # Get file dialog
    ...
    def fit_curve(self, kind='quadratic'):
        """
        Fits the curve given by a x_old and y_old, using the scipy.interpolate.interp1d package
        ...
        self.x_new = np.linspace(min(self.x_old), max(self.x_old), self.point_sample_size)
        ...
        self.plt.plot(np.log2(self.x_new + 0.001), self.y_new, '-', color=self.line_color)
